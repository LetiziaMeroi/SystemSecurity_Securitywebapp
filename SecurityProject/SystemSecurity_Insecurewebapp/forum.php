		...
		<div class = "content" style="text-align: center; margin-top: -25px;"> 
			...
			<?php 
				...
				if (isset($_SESSION['username'])){
					echo "<a href = '../SystemSecurity_Insecurewebapp/question.php'> <button class='btn btn-primary' name='submit'> Add a question! </button> </a>";
					
				}
			
				else {
					echo "<h5>You need to <a href = '../SystemSecurity_Insecurewebapp/login.php'>login</a> to ask a question!</h5>";
				}
			...
			if($result->num_rows > 0) {
				while ($row = $result-> fetch_assoc()){
					...
					echo " <tr>   
						<td>" .$row["titleQ"]."</td> 
						<td>" .$row["username"]."</td> 
						<td> <a href='../SystemSecurity_Insecurewebapp/selection.php?number=".$row["number"]."'> more </a> </td>
					</tr>";
				}
			}
			else {
